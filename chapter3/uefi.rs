...
pub struct EfiMemoryDescriptor {
    ...
    physical_start: u64, // 物理アドレス
    virtual_start: u64,  // 仮想アドレス
...
const MEMORY_MAP_BUFFER_SIZE: usize = 0x8000; // メモリマップのバッファサイズ

pub struct MemoryMapHolder {
    memory_map_buffer: [u8; MEMORY_MAP_BUFFER_SIZE],
    memory_map_size: usize, // メモリマップのサイズ
    map_key: usize, // メモリマップのキー
    descriptor_size: usize, // メモリディスクリプタのサイズ
    descriptor_version: u32, // メモリディスクリプタのバージョン
}
...
impl fmt::Write for VramTextWriter<'_> {
    fn write_str(&mut self, s: &str) -> fmt::Result {
        for c in s.chars() {
            if c == '\n' {
                self.cursor_y += 16; // 改行は 16 ピクセル下に移動
                self.cursor_x = 0; // カーソルを左端に戻す
                continue;
            }
            draw_font_fg(self.vram, self.cursor_x, self.cursor_y, 0xffffff, c);
            self.cursor_x += 8; // 1文字分右に移動
        }
        Ok(())
    }
}
